...
public class WirelessOSSHighPrecisionIntegerAddition2 {
	...
	public static String add(String str1, String str2, boolean flag) {
		...
		while (sb1.length() > 0 || sb2.length() > 0 || carry > 0) {
			...
			int num1 = (len1 == 0) ? 0 : Integer.parseInt(sb1.substring(sb1
					.length() - len1));
			int num2 = (len2 == 0) ? 0 : Integer.parseInt(sb2.substring(sb2
					.length() - len2));
		...
		if (flag)
			sb3 = new StringBuffer().append("-").append(sb3);
		return sb3.toString();
	}
