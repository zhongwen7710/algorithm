...
public final class LookingForArithmeticProgressionVersionTwo2 {
	private LookingForArithmeticProgressionVersionTwo2() {
	}
	...
	private static List<List<Integer>> getLongestArray(List<List<Integer>> ls) {
		...
		List<List<Integer>> lsMaxStep = new ArrayList<List<Integer>>();
		for (List<Integer> e : lsMax) {
			if (e.size() > 1) {
				if (e.get(1) - e.get(0) == maxStep) {
					lsMaxStep.add(e);
				}
			}
		}
	...
	private static void showList(String result) {
		String[] singleStr = result.split(" ");
